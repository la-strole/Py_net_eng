...
def ping_alice(alice_ip, router_ip, packet_count, max_wifi_delay, max_internet_delay, loss_persent,
    ...
    """ 
    Check Internet connection to yandex station with definrs delay, packge loss.
    alice_ip - yandex_station ip, ping router and alice 
    :return: csv file.
    """
    ...
    if result_ping_router.returncode == 0:
        ...
        # Check connection to trusted Internet address
        if result_ping_internet.returncode == 0:
            ...
            # Check connection to yandex station
            else:
...
def main():
    ...
    while True:
        with open('alice_ping.csv', 'a') as file:
